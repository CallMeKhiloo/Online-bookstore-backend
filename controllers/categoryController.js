...
const getCategory = async (req) => {
  ...
  if (!category) throw new Error('can not find category: id is not valid');
  return category;
};

const getBooksByCategory = async (req) => {
  const category = await Category.findById(req.params.id);
  if (!category) throw new Error('can not find category to show its books: id is not valid');
...
const updateCategory = async (req) => {
  ...
  if (!category) throw new Error('can not update category: id is not valid');
  return category;
}
